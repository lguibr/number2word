    ...
    <v-row justify="center">
      <v-col cols="12" xs="12" sm="8" md="6" lg="6" xl="6">
        <Card title="Number converted in words:">
          <CopyDisplay :copyText="numInWords"></CopyDisplay>
        </Card>
      </v-col>
    </v-row>
  </v-container>
...
export default {
  ...
  methods: {
    notifyError(error) {
      ...
      const defaultError = "Unknown error please enter in contact with the administrator "
    ...
    async fetchNumToWords() {
      ...
      await useFetch(`/api/num_to_english/${number}`, {
        onResponse({ response }) {
          if (response.ok) {
